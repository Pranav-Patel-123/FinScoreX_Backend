...
async def get_cibil_scores():
    try:
        ...
        all_months = sorted(set(doc.get("Month", "") for doc in raw_data if "Month" in doc))
        ...
        for doc in raw_data:
            ...
            business_data.setdefault(business_id, []).append(doc)

        # Sort records for each business by month
        for business_id, records in business_data.items():
            records.sort(key=lambda x: all_months.index(x["Month"]) if x.get("Month") in all_months else float('inf'))

            previous_score = None
            for doc in records:
                month = doc.get("Month", "")
                date = doc.get("Date", "")
                score = doc.get("AI_Credit_Score", 0)  # Default to 0 if missing
                ...
                formatted_data.append({
                    ...
                    "date": date,
                    "score": score,
                    "previousScore": previous_score if previous_score is not None else score
                })
